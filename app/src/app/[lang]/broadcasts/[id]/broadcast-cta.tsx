...
function parseLineId(raw: string): { type: 'oa' | 'id' | 'phone'; value: string; label: string; labelTh: string; display: string } {
  ...
  const label = type === 'oa' ? 'LINE Official Account' : type === 'phone' ? 'LINE Phone Number' : 'LINE ID';
  const labelTh = type === 'oa' ? 'LINE Official Account' : type === 'phone' ? 'LINE เบอร์โทรศัพท์' : 'LINE ID';
  const display = type === 'oa' ? `@${value}` : value;
  return { type, value, label, labelTh, display };
}
  ...
  return (
    <div style={{ background: 'white', borderRadius: '14px', border: '1px solid #E4E7ED', padding: '20px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        {lineInfo && (
          <div
            onClick={() => { track(broadcastId, 'click_line'); navigator.clipboard?.writeText(lineInfo.display).catch(() => {}); }}
            ...
            {LINE_ICON}
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '11px', color: '#3d9c40', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '2px' }}>
                {isTh ? lineInfo.labelTh : lineInfo.label}
              </div>
              <div style={{ fontSize: '15px', fontWeight: 700, color: '#171A21' }}>{lineInfo.display}</div>
            </div>
