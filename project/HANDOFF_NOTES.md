...
> **Scope note:** This handoff covers the product only. Expo booth artwork lives in `/booth/` (including `booth/EXPO_PITCH.md`) — out of scope for engineering.
...
> **Canonical source = the top-level `*.html` files listed below**, starting at `Landing Page.html`.
> ⚠️ **Ignore `ui_kits/` and `preview/`** — those are the original design-system starter/specimen files, superseded by these screens. Do not build from them.
